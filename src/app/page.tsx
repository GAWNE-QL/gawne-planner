...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <PlannerPortal store={store} onStore={updateStore} />
    </div>
  );
...
function PlannerPortal({ store, onStore }) {
  ...
  useEffect(() => {
    const sync = () => {
      ...
      setMobileId(m2 ? m2[1] : null);
      setCallSheetId(m3 ? m3[1] : null);
    };
    sync();
    addEventListener("hashchange", sync);
    return () => removeEventListener("hashchange", sync);
  }, []);
  const shootBy = id => store.shoots.find(s => s.id === id);
  if (mobileId) {
    const shoot = shootBy(mobileId);
    if (!shoot) return null;
    return <ShootDayMobile shoot={shoot} store={store} onStore={onStore} />;
  }
  if (callSheetId) {
    const shoot = shootBy(callSheetId);
    if (!shoot) return null;
    return <CallSheetPrint shoot={shoot} store={store} onStore={onStore} />;
  }
  const shoot = shootBy(openId);
  if (!openId || !shoot) return null;

  const tpl = store.templates.find(t => t.id === shoot.schemaTemplateId) || store.templates[0];
  const fields = tpl?.fields || [];
  const patch = p => onStore({ shoots: store.shoots.map(s => (s.id === shoot.id ? { ...s, ...p } : s)) });
  const patchScenes = n => patch({ scenes: n });
  const addScene = () => patchScenes([{ id: uid(), values: {}, title: "New Scene" }, ...(shoot.scenes || [])]);
  const duplicateScene = id => {
    const sc = (shoot.scenes || []).find(x => x.id === id);
    if (!sc) return;
    patchScenes([{ ...sc, id: uid() }, ...shoot.scenes]);
  };
  const removeScene = id => patchScenes((shoot.scenes || []).filter(s => s.id !== id));
  const updateSceneValue = (id, key, value) =>
    patchScenes((shoot.scenes || []).map(sc => (sc.id === id ? { ...sc, values: { ...sc.values, [key]: value } } : sc)));
  const duplicateShoot = () => {
    const copy = { ...shoot, id: uid(), name: `${shoot.name} (Copy)` };
    ...
    <Dialog open onOpenChange={v => { if (!v) location.hash = ""; }}>
      <DialogContent className="max-w-6xl w-[95vw] h-[90svh] max-h-[90svh] overflow-hidden p-0">
        <div className="flex flex-col h-full">
          ...
          <div className="px-4 pb-6 space-y-6 flex-1" style={bodyStyle}>
            <Card className="rounded-2xl shadow-sm"><CardHeader>
                <CardTitle>Overview</CardTitle>
              </CardHeader>
              <CardContent className="grid md:grid-cols-3 gap-4">
                <Field label={<span className="inline-flex items-center gap-2">Name <InfoTip text="Project name" /></span>}>
                  <Input value={shoot.name} onChange={e => patch({ name: e.target.value })} />
                </Field>
                <Field label={<span className="inline-flex items-center gap-2">Date/Time <InfoTip text="Leave empty = Vault" /></span>}>
                  <Input type="datetime-local" value={shoot.date} onChange={e => patch({ date: e.target.value })} />
                </Field>
                <Field label={<span className="inline-flex items-center gap-2">Duration (min) <InfoTip text="Expected time on set" /></span>}>
                ...
                <Field label={<span className="inline-flex items-center gap-2">Time of Day</span>}>
                  <Select value={shoot.timeOfDay} onValueChange={v => patch({ timeOfDay: v })}>
                    <SelectTrigger>
                ...
                <div className="md:col-span-3">
                  <Field label="General Notes">
                    <Textarea value={shoot.notes} onChange={e => patch({ notes: e.target.value })} />
                  </Field>
                </div>
              </CardContent>
            </Card>
                ...
                <div className="flex gap-2">
                  ...
                  <Popover>
                    ...
                    <PopoverContent align="end" className="w-44 p-2">
                      <Button size="sm" className="w-full mb-1" onClick={() => autoOrder("priority")}>
                        By Priority
                      </Button>
                      <Button size="sm" className="w-full" onClick={() => autoOrder("location")}>
                        By Location
                      </Button>
                    </PopoverContent>
                  </Popover>
              ...
              <CardContent className="space-y-3">
                {(shoot.scenes || []).map(sc => (
                  <SceneCard
                    ...
                    onChange={(k, v) => updateSceneValue(sc.id, k, v)}
                    onAddSongInline={s => onStore({ ...store, songs: [s, ...store.songs] })}
                  />
                ))}
                {(!shoot.scenes || shoot.scenes.length === 0) && (
                  <p className="text-sm text-muted-foreground">
                    No scenes yet. Click <span className="font-medium">Add Scene</span>.
                  </p>
                )}
              </CardContent>
            </Card>
...
function SceneCard({ sc, fields, songs, onDuplicate, onRemove, onChange, onAddSongInline }) {
  const render = f => {
    ...
    switch (f.kind) {
      ...
      case "textarea":
        return <Textarea value={v} onChange={e => onChange(f.key, e.target.value)} />;
      case "songs":
        return (
          <SongMultiSelect value={Array.isArray(v) ? v : []} options={songs} onChange={val => onChange(f.key, val)} onAdd={onAddSongInline} />
        );
      case "status":
        return (
          <Select value={v || STATUSES[0]} onValueChange={val => onChange(f.key, val)}>
            <SelectTrigger>
              <SelectValue placeholder="Status" />
            </SelectTrigger>
            <SelectContent>
      ...
      case "priority":
        return (
          <Select value={v || PRIORITIES[1]} onValueChange={val => onChange(f.key, val)}>
            <SelectTrigger>
      ...
      case "checklist":
        return <Checklist value={Array.isArray(v) ? v : []} onChange={val => onChange(f.key, val)} />;
      default:
        return <Input value={v} onChange={e => onChange(f.key, e.target.value)} />;
    }
  };
...
function SongMultiSelect({ value, options, onChange, onAdd }) {
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button variant="outline" className="justify-between w-full">
          <span className="truncate flex-1 text-left">{value.length ? value.join(" • ") : "Select songs"}</span>
          <Music2 className="h-4 w-4 ml-2" />
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-80 p-0" align="start">
        <div className="p-2 border-b space-y-2">
          ...
          <div className="flex gap-2">
            <Input placeholder="Add new song" value={newSong} onChange={e => setNewSong(e.target.value)} onKeyDown={e => { if (e.key === "Enter") add(); }} />
            <Button size="sm" onClick={add}>Add</Button>
          </div>
        </div>
        <div className="max-h-64 overflow-y-auto py-1">
          {list.map(opt => (
            <button key={opt} className="w-full text-left px-3 py-2 flex items-center gap-2 hover:bg-accent" onClick={() => toggle(opt)}>
              <Checkbox checked={value.includes(opt)} readOnly />
              <span>{opt}</span>
            </button>
          ))}
          {list.length === 0 && <div className="px-3 py-2 text-sm text-muted-foreground">No matches.</div>}
        </div>
                ...
                <div key={t.id} className="space-y-4">
                  ...
                  <div className="grid md:grid-cols-2 gap-3">
                    ...
                    <div className="md:col-span-2">
                      <Field label="Notes">
                        <Textarea value={t.overview?.notes || ""} onChange={e => update(t.id, { overview: { ...t.overview, notes: e.target.value } })} />
                      </Field>
                    </div>
                  </div>
          ...
          <div className="flex-1 overflow-y-auto px-4 pb-6" style={{ WebkitOverflowScrolling: "touch" }}>
            ...
            {(shoot.scenes || []).map(sc => (
              <div key={sc.id} className={`rounded-xl border p-3 mb-2 ${sc.done ? "opacity-70" : ""}`}>
                ...
                <div className="text-xs text-muted-foreground mt-1">{(sc.values?.description || "").slice(0, 140)}</div>
                <div className="mt-2 text-xs">Songs: {(Array.isArray(sc.values?.songs) ? sc.values.songs : []).join(" • ") || "—"}</div>
              </div>
            ))}
            {(!shoot.scenes || shoot.scenes.length === 0) && <div className="text-sm text-muted-foreground">No scenes yet.</div>}
          </div>
        </div>
      </DialogContent>
    </Dialog>
